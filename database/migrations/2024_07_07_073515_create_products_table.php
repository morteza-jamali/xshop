  ...
  public function up()
  {
    Schema::create('products', function (Blueprint $table) {
      ...
      $table
        ...
        ->nullable()
        ->comment(
          'Quick summary for product. This will appear on the product page under the product name and for SEO purpose.',
        );
      $table
        ->string('sku')
        ->nullable()
      ...
      $table
        ...
        ->index()
        ->comment(
          'If this product is a non-physical item, for example a service, which does not need shipping.',
        );
      $table
        ...
        ->index()
        ->comment(
          'If purchasing this product gives a customer access to a downloadable file, e.g. software',
        );
      ...
      $table
        ->enum('stock_status', ['IN_STOCK', 'OUT_STOCK', 'BACK_ORDER'])
        ->nullable()
        ->default('IN_STOCK')
        ->index();
      $table->unsignedBigInteger('rating_count')->nullable()->default(0);
      $table->unsignedDecimal('average_rating', 3, 2)->nullable()->default(0.0);
